...
##### 모델 결과 관리 파일 #####
    ...
    2) Import Library (Import 해야하는 모듈) 
        ...
        2-2) (Common 폴더 내) rum_rslt_Excel.py   # cas2 2) DB 활용 불가능한 분석 환경 
...
##### 환경설정에 필요한 파일 #####  
      ...
      3) Common 폴더
          ...
          3-2) util.py   ( 환경설정을 위한 공통 디렉토리 함수 )


      4) Conf(Config) 폴더 

          4-1) Config.yaml 파일 (DB Connection을 위한 정보 포함)

      5) lib 폴더

          5-1) JDBC Driver 를 다운받아 (.jar) file 형태로 저장 
...
![JDBC](https://user-images.githubusercontent.com/104196127/165456007-0d49a61c-ae3d-452b-80b9-24bfa19ff342.jpg)
...
![config](https://user-images.githubusercontent.com/104196127/165472568-7c99848a-096a-4408-8b34-2fb1a1ecf9d5.jpg)
...
##### **1) run_rslt.py 열기** #####
       ...
       # [Screen_shot_1] #  
        ...
        - 지정된 상위 폴더(root_path) 내 run_rslt.py 파일을 모델 폴더(basename)에 이동합니다. 
                - 파일명은 자유롭게 변경 가능합니다. 
...
###### 파일 디렉토리 설명 ######
      ...
      * root_path : (모델에 관계없이) 공통적인 분석 환경 세팅을 위한 상위 폴더의 경로입니다.
      * basename  : root_path 내에 존재하는 여러 분석 모델 폴더 중 특정 분석 모델에 대한 폴더 
                                                                  (ex.모델/데이터 셋/평가지표 등) 
...
![모델_코드_삽입구간](https://user-images.githubusercontent.com/104196127/165452218-978e00eb-74c8-4fc6-aa21-76c4884911c5.jpg)


      - 모델 실행에 필요한 코드를 지정된 위치에 삽압힙니다.
              * Library : 모델을 위한 Import 패키지 
              * 모델 전체 코드 : 모델 실행을 위한 코드 전체 
