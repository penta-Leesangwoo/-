...
##### 코드 실행 순서 #####
    ...
    1) run_rslt.ipynb 열기
       ...
       # [Screen_shot_1] #  
        - 지정된 root_path 내 run_rslt.ipynb 파일을 모델 폴더(basename)에 복사(copy) 합니다.   
                                                                        (* root_path 와 basename 설명은 아래 '파일 디렉토리 설명' 참조)

##### 파일 디렉토리 설명 #####

  * root_path : (모델에 관계없이) 공통적인 분석 환경 세팅을 위한 상위 폴더의 경로입니다.
  * basename  : root_path 내에 존재하는 여러 분석 모델 폴더 중 특정 분석 모델에 대한 정보(ex.모델 실행 파일/ 데이터 셋/plot 폴더 등) 를 담고있는 폴더의 경로입니다.   

  (common 폴더 내 util.py Import 한 경우) 분석 환경 내 디렉토리를 관리하기 위해 자동으로 폴더 디렉토리를 설정해줍니다. 
                                             
                                             
    
    2) DB 결과 관리 모듈 Import
    
      - 결과 관리 모듈을 Import 합니다. (파일 내 코드 구성되어 있음)
      - from common.run_rslt_DB import model_run_rslt
    ...
    3) 관리하고자 하는 모델 명칭과 기준 날짜 변수 할당 
      ...
      - 저장하고자 하는 모델 정보 (명칭,실행날짜)를 기입합니다.  
    ...
    4) 모델 실행 코드 삽입 
        ...
        - model 함수 내 Sample code 부분을 지운 뒤, 분석 모델링 코드 전체를 삽입합니다.
...
- DB 활용이 불가능하지만 결과 이력관리가 필요한 경우 사용합니다. <br/>
